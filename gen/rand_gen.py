...
def sample(args):
    ...
    # Random sampling 
    sample_cad = []
    while len(sample_cad) < RANDOM_SAMPLE_TOTAL:
        # Sample code
        codes = codePred.sample(n_samples=RANDOM_SAMPLE_BS).detach().cpu().numpy()
        ...
        # Sample CAD
        xy_samples, _latent_code_, _codes_pad_mask_ = tokenPredS.sample(codes_pad, codes_pad_mask)
        assert len(xy_samples) == len(_latent_code_)
        assert len(_codes_pad_mask_) == len(_latent_code_)
        cad_samples = tokenPredE.sample(_latent_code_, _codes_pad_mask_, xy_samples)
    ...
    # Compute Novel and Unique scors
    non_novel_count = 0 
    gen_count = 0
    non_unique_count = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--code_weight", type=str, help="Pretrained code-tree model")
    parser.add_argument("--cad_weight", type=str, help="Pretrained CAD model")
    parser.add_argument("--output", type=str, help="Output folder to save the data")
    parser.add_argument("--device", type=str, help="CUDA Device Index")
    parser.add_argument("--solid_code", type=str)
    parser.add_argument("--profile_code", type=str)
    parser.add_argument("--loop_code", type=str)
